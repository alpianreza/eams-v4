...
Setup Docker lengkap: **PHP 8.5-FPM + Nginx + MariaDB 10.4 + Queue worker + Scheduler**. Satu perintah untuk jalan.
...
| `db` | mariadb:10.4 | database (match produksi) | internal :3306 |
...
Volume: `appcode` (kode + storage, persisten) dan `dbdata` (data MariaDB). File upload tersimpan di `appcode` → `storage/`.
