...
<div x-data="{ failed: false }" {{ $attributes->class('eams:relative eams:overflow-hidden eams:rounded-eams-lg eams:border eams:border-border eams:bg-surface-sunk') }} data-eams-component="image-preview">
    @if($src)
        <img x-show="!failed" @error="failed = true" src="{{ $src }}" alt="{{ $alt }}" @class(['eams:w-full eams:object-cover', 'eams:aspect-video' => $ratio === 'video', 'eams:aspect-square' => $ratio === 'square'])>
    @endif
    <div @if($src) x-show="failed" x-cloak @endif @class(['eams:flex eams:w-full eams:flex-col eams:items-center eams:justify-center eams:p-6 eams:text-center eams:text-muted', 'eams:aspect-video' => $ratio === 'video', 'eams:aspect-square' => $ratio === 'square'])>
